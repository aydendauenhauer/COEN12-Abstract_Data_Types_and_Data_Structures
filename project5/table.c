...
void quicksort(void *elts[], int min, int max, int (*compare)());
void *median(void *elts[], int (*compare)(), int min, int max);
int partition(void *elts[], int (*compare)(), int min, int max);
void swap(void *elts[], int i, int j);
 ...
 *
 * Description:	The following functions are used for a quick sort algorithm
 */
...
// To determine the elements in the middle of the array
// Runtime: O(1)
void *median(void *elts[], int (*compare)(), int min, int max) {
        int mid = (max + min) / 2;

        if(compare(elts[min], elts[max]) > 0) {
                swap(elts, min, max);
        }

        if(compare(elts[min], elts[mid]) > 0) {
                swap(elts, min, mid);
        }

        if(compare(elts[mid], elts[max]) > 0) {
                swap(elts, mid, max);
        }
...
// To place elements on the left or right side of the index depending on whether it's smaller or larger than it
// Runtime: O(n)
int partition(void *elts[], int (*compare)(), int min, int max) {
        void *pivot = median(elts, compare, min, max);

        while(min < max) {
                while(compare(elts[min], pivot) < 0) {
                        min++;
                }

                while(compare(elts[max], pivot) > 0) {
                        max--;
                }
...
// To enact the sorting on the array
// Runtime: O(1)
void quicksort(void *elts[], int min, int max, int (*compare)()) {
        if(min < max) {
                int pivot = partition(elts, compare, min, max);
                quicksort(elts, min, pivot - 1, compare);
                quicksort(elts, pivot + 1, max, compare);
        }
}
